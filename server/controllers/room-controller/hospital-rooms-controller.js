...
export const createHospitalRoom = async (req, res) => {
  ...
  let sql = CREATE_NEW_ROOM_QUERY;
    ...
    if (!validateNumberField(room_number)) {
      return res
          .status(400)
          .json({ msg: "Invalid value for room type" });
    }
...
export const editRoom = async (req, res) => {
  ...
  con.query(GET_ROOMS_WITH_APPOINTMENTS, [room_number], (err, result) => {
    if (result.length > 0) {
      return res
          .status(404)
          .json({ msg: "Error! There are still scheduled appointments in this room" });
    }
  })

  con.query(GET_ROOM_BY_ROOMNUMBER_OR_TYPE, [room_number], [type] ,  (err, result) => {
    if (result.length > 0) {
      return res
          .status(404)
