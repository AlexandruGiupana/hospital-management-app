...
import {
  ...
  SELECT_ALL_REPARTITIONS, SELECT_REPARTITION_BY_ID_QUERIES
...
export const assignMedicalServiceToDoctor = async (req, res) => {
  ...
  if(!validateNumberField(service_id)) {
    return res
      .status(400)
      .json({ msg: "Invalid value for service" });
  }
  if(!validateNumberField(doctor_id)) {
    return res
      .status(400)
      .json({ msg: "Invalid value for doctor" });
  }
  con.query(CHECK_DOCTOR_ALREADY_ASSIGNED_QUERY, [service_id, doctor_id], (err, result) => {
    if (result.length !== 0) {
      return res
        .status(409)
      ...
      con.query(SELECT_DOCTOR_BY_ID_QUERY, [doctor_id], (err, result) => {
        if (result.length === 0) {
          ...
          con.query(SELECT_SERVICE_BY_ID_QUERY, [service_id], (err, result) => {
            if (result.length === 0) {
              return res
                .status(404)
                .json({ msg: "Service does not exist" });
            } else {
              con.query(ADD_NEW_REPARTITION_QUERY, [service_id, doctor_id], (err, result) => {
                if (err) {
...
export const deleteRepartition = async (req, res) => {
  ...
  con.query(SELECT_REPARTITION_BY_ID_QUERIES, [id], (err, result) => {
    if(result.length === 0) {
      return res
        .status(404)
        .json({ msg: "Repartition does not exist" });
    } else {
